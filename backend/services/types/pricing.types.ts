...
export interface ChildTouristTaxRate {
  childRateId: string;              // UUID for tracking
  ageFrom: number;                  // 0-16 (inclusive)
  ageTo: number;                    // 1-17 (inclusive, must be > ageFrom)
  amount: number;                   // Tax amount (can be 0)
  displayLabel: {                   // Auto-generated by backend
...
export interface TouristTax {
  ...
  // NEW: Age-based child rates (replaces single childAmount)
  childRates: ChildTouristTaxRate[];
  
  // DEPRECATED: Legacy single child amount (kept for backward compatibility)
  // Will be automatically converted to childRates on read
  childAmount?: number;
}
...
export interface SetPricingRequest {
  ...
  touristTax?: {
    ...
    childRates: Array<{                // Age-based child rates (required if touristTax provided)
      ageFrom: number;                 // 0-16 (inclusive)
      ageTo: number;                   // 1-17 (inclusive)
      amount: number;                  // Tax amount (can be 0)
      displayLabel: {
        en: string;
        sr: string;
